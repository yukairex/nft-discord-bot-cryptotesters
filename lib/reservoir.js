...
const getSales = async (address, startTimestamp=0,endTimestamp=1681563474, apiKey=process.env.RESERVIOR_APIKEY) => {
    ...
    const url = `https://api-optimism.reservoir.tools/sales/v4?includeDeleted=false&collection=${address}&orderBy=time&sortDirection=asc&startTimestamp=${startTimestamp}&&endTimestamp=${endTimestamp}=1681563474&limit=10`
    console.log(url);
    try { 
...
const getFloors = async (address, apiKey=process.env.RESERVIOR_APIKEY) => {
   ...
   // https://api-optimism.reservoir.tools/collections/v5?id=0x18a1bC18cEfdc952121F319039502FDD5f48B6fF&includeTopBid=false&normalizeRoyalties=false&useNonFlaggedFloorAsk=false&sortBy=allTimeVolume&limit=20
    ...
    try { 
        const url = `${endpoint}collections/v5?id=${address}&includeTopBid=false&normalizeRoyalties=false&useNonFlaggedFloorAsk=false&sortBy=allTimeVolume&limit=1`
        const data = await axios.get(url, {
            headers: {
                'x-api-key': apiKey
              }
